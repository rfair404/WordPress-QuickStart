...
test.describe( 'WordPress Frontend @wordpress', () => {
	...
	test( 'navigation menu works', async ( { page } ) => {
		...
		if ( ! navigationFound ) {
			console.log(
				'ℹ️  No navigation menu found, checking basic page structure'
			);
			await expect( page.locator( 'body' ) ).toBeVisible();
		}
	...
	test( '404 page handles non-existent URLs', async ( { page } ) => {
		const nonExistentUrl =
			'/this-page-definitely-does-not-exist-' + Date.now();
		...
		const response = await page.waitForResponse(
			( response ) =>
				response.url().includes( nonExistentUrl ) &&
				response.status() === 404
		);
