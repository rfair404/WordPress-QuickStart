...
test.describe('WordPress Admin @wordpress', () => {
  ...
  test('can save post as draft', async ({ page }) => {
    ...
    await expect(page.locator(`text=Draft ${testData.postTitle}`)).toBeVisible();

    console.log('✅ Draft post saved successfully');
  });

  test('plugins page loads and shows installed plugins', async ({ page }) => {
    await admin.login();
  ...
  test('themes page loads and shows available themes', async ({ page }) => {
    await admin.login();
  ...
  test('user profile can be accessed and updated', async ({ page }) => {
    ...
    // Update display name
    const testData = testUtils.generateTestData();
    await page.fill('#display_name', `${testData.firstName} ${testData.lastName}`);
  ...
  test('admin notices are displayed properly', async ({ page }) => {
    ...
    for (const notice of notices) {
      ...
      expect(['error', 'warning', 'success', 'info']).toContain(notice.type);
    }
  ...
  test('admin area has proper security headers', async ({ page }) => {
    ...
    const hasNonce = await testUtils.elementExists('input[name*="_wpnonce"], input[name*="_nonce"]');
    ...
    for (const form of forms) {
      ...
      if (action && action.length > 0) {
        const isValidAction = action.includes('admin-post.php') ||
  ...
  test('block editor (Gutenberg) loads correctly', async ({ page }) => {
    ...
    await expect(page.locator('.editor-post-title__input')).toBeVisible();
    await expect(page.locator('.block-editor-default-block-appender')).toBeVisible();

    // Check for block inserter
    await expect(page.locator('.block-editor-inserter__toggle')).toBeVisible();
