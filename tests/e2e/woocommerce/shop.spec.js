...
test.describe('WooCommerce Shop @woocommerce', () => {
  ...
  test('shop page loads correctly', async ({ page }) => {
    ...
    if (!shopElementFound) {
      const hasWooCommerceContent = await testUtils.elementExists('.woocommerce');
  ...
  test('product search works', async ({ page }) => {
    ...
    const hasResults = await testUtils.elementExists('.woocommerce-products-header') ||
  ...
  test('cart page is accessible', async ({ page }) => {
    ...
    await expect(page.locator('.woocommerce-cart-form, .cart-empty')).toBeVisible();
  ...
  test('checkout page is accessible', async ({ page }) => {
    ...
    const checkoutContent = await testUtils.elementExists('.woocommerce-checkout-payment') ||
  ...
  test('my account page loads', async ({ page }) => {
    ...
    const hasAccount = await testUtils.elementExists('.woocommerce-MyAccount-navigation');
  ...
  test('WooCommerce widgets and blocks work', async ({ page }) => {
    ...
    if (!wcElementFound) {
      await wpAssertions.assertWooCommerceActive();
      console.log('✅ WooCommerce is active (no specific widgets/blocks found)');
    }
  });
  ...
  test.skip('can add product to cart', async ({ page }) => {
    ...
    try {
      ...
      console.log('ℹ️  Product add to cart test skipped - no test products available');
    }
  });
  ...
  test.skip('checkout process works', async ({ page }) => {
    ...
    try {
      ...
      console.log('ℹ️  Checkout test skipped - requires complete WooCommerce setup');
    }
  });
});
...
test.describe('WooCommerce Admin @woocommerce', () => {
  ...
  test('WooCommerce admin pages are accessible', async ({ page }) => {
    ...
    for (const wcPage of wcAdminPages) {
      try {
        ...
        await page.waitForSelector('.wrap, #wpbody-content', { timeout: 5000 });

        const hasContent = await testUtils.elementExists('.wrap') ||
        ...
        console.log(`ℹ️  ${wcPage.name} page not found (WooCommerce may not be installed)`);
      }
    }

    if (wcPagesFound === 0) {
      console.log('ℹ️  No WooCommerce admin pages found - WooCommerce may not be installed');
  ...
  test.skip('can create a simple product', async ({ page }) => {
    ...
    try {
      ...
      // Wait for success
      await page.waitForSelector('.updated, .notice-success', { timeout: 10000 });

      console.log('✅ Product created successfully');
    } catch (error) {
      console.log('ℹ️  Product creation test skipped - WooCommerce not available');
    }
  });
  ...
  test('WooCommerce installation check', async ({ page }) => {
    ...
    const hasWooCommerce = await testUtils.elementExists('[data-slug="woocommerce"]');

    if (hasWooCommerce) {
      console.log('✅ WooCommerce plugin is installed');

      // Check if it's active
      const isActive = await testUtils.elementExists('[data-slug="woocommerce"].active');
      if (isActive) {
      ...
      console.log('ℹ️  WooCommerce plugin not found - install it to enable e-commerce functionality');
    }
  });
});
