...
test.describe( 'WooCommerce Shop @woocommerce', () => {
	...
	test( 'checkout page is accessible', async ( { page } ) => {
		...
		const checkoutForm = await testUtils.elementExists(
	...
	test( 'my account page loads', async ( { page } ) => {
		...
		const hasLogin = await testUtils.elementExists(
	...
	test( 'product categories page works', async ( { page } ) => {
			...
			try {
				...
				if ( response.status() === 200 ) {
					...
					if ( await testUtils.elementExists( '.woocommerce' ) ) {
						categoryFound = true;
						console.log(
							`✅ Product category page works: ${ url }`
						);
						break;
					}
				}
		...
		if ( ! categoryFound ) {
			...
			await wpAssertions.assertWooCommerceActive();
			console.log(
				'✅ Shop page verified (no specific categories found)'
			);
		}
	} );
	...
	test( 'WooCommerce installation check', async ( { page } ) => {
		...
		if ( hasWooCommerce ) {
			...
			if ( isActive ) {
				...
				console.log(
					'ℹ️  WooCommerce plugin is installed but not active'
				);
			}
		} else {
			console.log(
