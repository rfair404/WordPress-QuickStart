...
class WooCommerceShop {
	constructor( page ) {
		this.page = page;
		this.baseURL =
			page.context()._options.baseURL || 'http://localhost:8080';
	}
	...
	async addToCart( productName, quantity = 1 ) {
		...
		if ( await productElement.isVisible() ) {
			...
			await productElement
				.locator( '.add_to_cart_button, button[name="add-to-cart"]' )
				.first()
				.click();
	...
	async proceedToCheckout() {
		...
		await this.page.waitForSelector( '.woocommerce-checkout', {
			timeout: 10000,
		} );
	}
	...
	async selectPaymentMethod( method = 'cod' ) {
		const paymentOption = this.page.locator(
	...
	async placeOrder() {
		...
		const orderReceived = await this.page
			.locator( '.woocommerce-order-received' )
			.isVisible();
	...
	/**
	 * Get cart totals
	 * @return {Object} Cart totals information
	 */
	async getCartTotals() {
		await this.viewCart();

		const subtotal = await this.page
			.locator( '.cart-subtotal .amount' )
			.textContent();
	...
	async getCartItemCount() {
		const cartCount = await this.page
			.locator( '.cart-contents-count, .cartcontents' )
			.textContent();
	...
	async loginCustomer( username, password ) {
		...
		await this.page.waitForSelector(
			'.woocommerce-MyAccount-navigation, .woocommerce-error',
			{ timeout: 10000 }
		);
