...
class WordPressAdmin {
  ...
  async installPlugin(pluginSlug) {
    ...
    // Install plugin
    const installButton = this.page.locator(`.plugin-card-${pluginSlug} .install-now`).first();
    if (await installButton.isVisible()) {
      await installButton.click();

      // Wait for installation to complete
      await this.page.waitForSelector('.activate-now', { timeout: 30000 });

      // Activate plugin
      await this.page.click('.activate-now');

      console.log(`✅ Successfully installed and activated plugin: ${pluginSlug}`);
    } else {
  ...
  async activateTheme(themeSlug) {
    ...
    if (await themeElement.isVisible()) {
      ...
      // Wait for activation
      await this.page.waitForSelector('.current-theme', { timeout: 10000 });
  ...
  async createPost({ title, content, status = 'publish' }) {
    ...
    await this.page.waitForSelector('.block-editor-writing-flow', { timeout: 15000 });
    ...
    await this.page.fill('.block-editor-default-block-appender .wp-block-paragraph', content);
    ...
    if (status === 'publish') {
      ...
      await this.page.waitForSelector('.components-snackbar__content', { timeout: 10000 });
   ...
   * @returns {Array} Array of notice messages
   */
