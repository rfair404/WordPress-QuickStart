...
class TestUtils {
  ...
  async waitForWordPress() {
    ...
    try {
      await this.page.waitForSelector('.wp-site-blocks, #main, .site-main', { timeout: 5000 });
    } catch (error) {
      // Continue if WordPress-specific elements aren't found
      console.warn('WordPress-specific elements not found, continuing...');
    }
  }
  ...
  async handleLoginRedirects() {
    ...
    if (currentUrl.includes('wp-login.php') && !currentUrl.includes('loggedout=true')) {
...
class WordPressAssertions {
  ...
  async assertWordPressLoaded() {
    ...
    if (!found) {
      ...
      const generator = await this.page.locator('meta[name="generator"][content*="WordPress"]').isVisible();
      if (generator) {
        found = true;
      }
    }
