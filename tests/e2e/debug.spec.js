...
test.describe('Debugging Utilities', () => {
    ...
    // Check for WordPress
    const isWordPress = await page.locator('meta[name="generator"][content*="WordPress"]').isVisible() ||
      await page.locator('body[class*="wp-"]').isVisible();
    console.log(`WordPress Detected: ${isWordPress}`);

    // Check for WooCommerce
    const isWooCommerce = await page.locator('body.woocommerce').isVisible() ||
  ...
  test('WordPress health check', async ({ page }) => {
    ...
    const healthChecks = [
      { name: 'Homepage loads', check: () => page.locator('body').isVisible() },
      { name: 'WordPress meta tag', check: () => page.locator('meta[name="generator"][content*="WordPress"]').isVisible() },
      { name: 'WordPress body class', check: () => page.locator('body[class*="wp-"]').isVisible() },
      {
        name: 'Admin login accessible', check: async () => {
          await page.goto('/wp-login.php');
          return page.locator('#loginform').isVisible();
        }
      },
      {
        name: 'REST API endpoint', check: async () => {
          const response = await page.request.get('/wp-json/');
          return response.status() === 200;
        }
      },
      {
        name: 'RSS feed accessible', check: async () => {
  ...
  test('WooCommerce health check', async ({ page }) => {
    ...
    const wcHealthChecks = [
      {
        name: 'Shop page accessible', check: async () => {
          await shop.goToShop();
          return testUtils.elementExists('.woocommerce');
        }
      },
      {
        name: 'Cart page accessible', check: async () => {
          await page.goto('/cart');
          return testUtils.elementExists('.woocommerce-cart-form') || testUtils.elementExists('.cart-empty');
        }
      },
      {
        name: 'Checkout page accessible', check: async () => {
          await page.goto('/checkout');
          return testUtils.elementExists('.woocommerce-checkout') || page.url().includes('checkout');
        }
      },
      {
        name: 'My Account page accessible', check: async () => {
          await page.goto('/my-account');
          return testUtils.elementExists('.woocommerce-form-login') || testUtils.elementExists('.woocommerce-MyAccount-navigation');
        }
      },
      {
        name: 'WooCommerce REST API', check: async () => {
  ...
  test('console error monitoring', async ({ page }) => {
    ...
    if (consoleMessages.length > 0) {
      console.log('Recent Console Messages:', JSON.stringify(consoleMessages.slice(-5), null, 2));
    }
  ...
  test('trace generation for debugging', async ({ page }) => {
    ...
    try {
      ...
      console.log('   View with: npx playwright show-trace test-results/debugging-trace.zip');
    }
  });
});
