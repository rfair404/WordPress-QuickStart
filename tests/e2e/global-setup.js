// Global setup for WordPress E2E tests
  ...
  try {
    ...
    while (retries > 0 && !isReady) {
      try {
        ...
        console.log(`⏳ Waiting for WordPress... (${retries} attempts left)`);
        retries--;
    ...
    try {
      ...
      console.warn('⚠️  Some WordPress components may not be fully ready:', error.message);
    }

    // Store test data in storage state for authentication tests
    await page.context().storageState({ path: './test-results/auth-setup.json' });
